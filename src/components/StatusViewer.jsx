...
export default function StatusViewer({ status, onClose, onNextUser }) {
  ...
  const currentStorypicture = currentStory.image ? currentStory.image : currentStory.text;

  const currentStorytime = new Date(currentStory.createdAt).toLocaleTimeString([], {
  hour: '2-digit',
  minute: '2-digit',
  hour12: true
});
  ...
  useEffect(() => {
    if (isImageLoading && currentStory?.image) return; 
  ...
  }, [currentIndex, status.id, isImageLoading, currentStory]);
  ...
  return (
    <div className="relative w-full max-w-[450px] h-full md:h-[95vh] bg-black md:rounded-2xl overflow-hidden flex flex-col shadow-2xl">
      ...
      <div className="absolute top-8 left-0 w-full px-4 flex items-center justify-between z-50 text-white bg-gradient-to-b from-black/50 to-transparent pb-10">
        <div className="flex items-center gap-3">
          {currentStorypicture?.startsWith("http") ? (
    <img
      src={currentStorypicture}
      className="w-10 h-10 rounded-full border border-white/40 object-cover"
      alt=""
    />
  ) : (
    <div className="w-10 h-10 rounded-full border border-white/40 flex items-center justify-center bg-gray-600 text-white text-[10px] font-bold p-1 text-center">
<p>      {currentStorypicture}
</p>
    </div>
  )} <div>
            <p className="font-bold text-sm shadow-sm">{status.name}</p>
      ...
      <div className="flex-1 bg-zinc-900 flex items-center justify-center relative">
        ...
        {isImageLoading && currentStory?.image && (
        ...
        {currentStory?.image ? (
          ...
          <div className="w-full h-full flex items-center justify-center p-10 bg-gradient-to-br from-purple-600 to-blue-700 text-white text-center"
               onPointerOver={() => setIsImageLoading(false)} 
               ref={() => setTimeout(() => setIsImageLoading(false), 100)}> 
        ...
        {!isImageLoading && currentStory?.text && currentStory?.image && (
          <div className="absolute bottom-10 left-0 w-full px-6 py-4 bg-black/40 backdrop-blur-sm text-white text-center z-50">
